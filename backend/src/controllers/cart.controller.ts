...
export async function totals(req: Request, res: Response) {
  const result = await service.getTotals(req.user!.id);
  res.json(result);
}
...
export async function updateItem(req: Request, res: Response) {
  const result = await service.updateItem(req.user!.id, req.params.productId!, req.body.quantity);
  res.json(result);
}

export async function removeItem(req: Request, res: Response) {
  const result = await service.removeItem(req.user!.id, req.params.productId!);
  res.json(result);
}
