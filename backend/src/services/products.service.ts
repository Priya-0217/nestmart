...
export async function listProducts(args: ListProductsArgs) {
  ...
  if (args.category) {
    const categoryDoc = Types.ObjectId.isValid(args.category)
      ? await CategoryModel.findById(args.category)
      : await CategoryModel.findOne({
          $or: [
            { slug: { $regex: new RegExp(`^${args.category}$`, "i") } },
            { name: { $regex: new RegExp(`^${args.category}$`, "i") } },
          ],
        });

    if (categoryDoc) {
      // Robust filter to handle data inconsistencies: matches ID, slug, or name case-insensitively
      filter.$or = [
        { category: categoryDoc._id },
        { category: { $regex: new RegExp(`^${categoryDoc.slug}$`, "i") } },
        { category: { $regex: new RegExp(`^${categoryDoc.name}$`, "i") } },
      ];
    } else {
      // Fallback: try direct match with whatever was passed (ID or case-insensitive string)
      if (Types.ObjectId.isValid(args.category)) {
        filter.category = new Types.ObjectId(args.category);
      } else {
  ...
  if (args.isFeatured !== undefined) filter.isFeatured = args.isFeatured;
