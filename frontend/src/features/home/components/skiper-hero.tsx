...
export function SkiperHero({ slides, className }: SkiperHeroProps) {
  ...
  const imageInitial = {
    opacity: 0,
    clipPath: reduceMotion ? 'inset(0% 0% 0% 0% round 28px)' : 'inset(12% 18% 12% 18% round 28px)',
      ...
      <div className="relative overflow-hidden rounded-[1.5rem] bg-card">
        <div className="grid items-center gap-4 px-3 py-4 sm:px-5 sm:py-6 md:grid-cols-[1.1fr_0.9fr] md:gap-8 md:px-8 md:py-8">
          <AnimatePresence mode="wait">
            ...
            >
              <motion.div initial={reduceMotion ? 'show' : 'hidden'} animate="show" variants={textContainer}>
                <motion.p
                  variants={textItem}
